...
verify_valid_path_variables() {
    local dir_vars=("BACKUP_DIR" "RADARR_DIR")
...
# Verify that $BACKUP_DIR and $RADARR_DIR are valid paths.
verify_valid_path_variables
...
echo_ts "[RADARR TARBALL BACKUP STARTED]"
...
current_backup_size=$(ls -lh "$RADARR_TAR_FILE" | awk '{ print $5 }')
