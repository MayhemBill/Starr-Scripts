...
OVERSEERR_TARFILE_TEXT="Overseerr-Backup"
TIMESTAMP() { date +"%Y_%m_%d"; }
COMPLETE_OVERSEERR_TARFILE_NAME() { echo "[$(TIMESTAMP)] $OVERSEERR_TARFILE_TEXT.tar"; }
OVERSEERR_TAR_COMMAND() {
    tar -cf "$OVERSEERR_TAR_FILE" "db" "settings.json"
}
#########################################################
################## END OF USER CONFIG ###################
##########################################################
...
verify_valid_path_variables() {
    local dir_vars=("BACKUP_DIR" "OVERSEERR_DIR")
...
# Function to create the overseerr tar file.
create_overseerr_tar_file() {
    # Navigate to $OVERSEERR_DIR working direcotry.
    cd "$OVERSEERR_DIR"
    # Create sub-directory name with the custom timestamp.
    OVERSEERR_TAR_FILE="$BACKUP_DIR/$(COMPLETE_OVERSEERR_TARFILE_NAME)"
    echo_ts "Creating Overseerr tar file..."
    # Run the tar command.
    OVERSEERR_TAR_COMMAND
    echo_ts "Overseerr Tar file created."
}

set_permissions() {
    echo_ts "Running 'chmod $PERMISSIONS' on tar files..."
    if [[ $OVERSEERR_ENABLED = true ]]; then chmod $PERMISSIONS "$OVERSEERR_TAR_FILE"; fi
...
# Verify that $BACKUP_DIR and $OVERSEERR_DIR are valid paths.
verify_valid_path_variables
...
echo_ts "[OVERSEERR TARBALL BACKUP STARTED]"
# Create the tar files.
if [[ $OVERSEERR_ENABLED = true ]]; then create_overseerr_tar_file; fi
# Set permissions for the tar files.
if [[ $PERMISSIONS =~ ^[0-9]{3,4}$ ]]; then set_permissions; fi

# Backup completed message.

current_backup_size=$(ls -lh "$OVERSEERR_TAR_FILE" | awk '{ print $5 }')
