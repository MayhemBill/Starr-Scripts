...
if [[ $OVERSEERR_ENABLED = true ]]; then source "$(dirname "$BASH_SOURCE")"/overseerr.sh; fi
...
send_notifiarr_webhook() {
    if [ $USE_NOTIFIARR = true ]; then
        ...
        python3 "$(dirname "$BASH_SOURCE")"/../misc/notifiarr.py -e "Starr DB Backup" -c 1164585523348783104 -m "FFA500" -t "Backup Success" -g "[{$lidarr_notification}, {$overseerr_notification}, {$prowlarr_notification}, {$radarr_notification}, {$sonarr_notification}]" -b "Backup was created at '$BACKUP_DIR'" -a "$ICON" -z "MYHM.Space"
    fi
}
