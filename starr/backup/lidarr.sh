...
verify_valid_path_variables() {
    local dir_vars=("BACKUP_DIR" "LIDARR_DIR")
...
# Function to create the lidarr tar file.
create_lidarr_tar_file() {
    # Navigate to $LIDARR_DIR working direcotry.
    cd "$LIDARR_DIR"
...
# Verify that $BACKUP_DIR and $LIDARR_DIR are valid paths.
verify_valid_path_variables
...
echo_ts "[LIDARR TARBALL BACKUP STARTED]"
...
current_backup_size=$(ls -lh "$LIDARR_TAR_FILE" | awk '{ print $5 }')
