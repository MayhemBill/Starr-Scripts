...
verify_valid_path_variables() {
    local dir_vars=("BACKUP_DIR" "SONARR_DIR")
...
# Verify that $BACKUP_DIR and $SONARR_DIR are valid paths.
verify_valid_path_variables
...
echo_ts "[SONARR TARBALL BACKUP STARTED]"
...
current_backup_size=$(ls -lh "$SONARR_TAR_FILE" | awk '{ print $5 }')
